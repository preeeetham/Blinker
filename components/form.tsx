...
const Form: React.FC<FormProps> = ({
  ...
  const handlePreview = async () => {
    ...
    const recipientPubKey = new PublicKey(
      process.env.NEXT_PUBLIC_WALLET || "8twrkXxvDzuUezvbkgg3LxpTEZ59KiFx2VxPFDkucLk3",
    )
      ...
      <div className="card md:p-10 h-full" ref={form}>
        {showForm && (
          <div className="space-y-6 h-full">
            <h1 className="text-2xl md:text-3xl font-bold mb-6 gradient-text">Customize Your Blink</h1>
            ...
            <div>
              ...
              <textarea
                ...
                maxLength={143}
              />
              <p className="text-xs text-[var(--text-secondary)] mt-1">{description.length}/143 characters</p>
            </div>

            {publicKey ? (
              <button className="button-primary w-full mt-4" onClick={handlePreview} disabled={!connected}>
                Generate Blink
              </button>
            ) : (
