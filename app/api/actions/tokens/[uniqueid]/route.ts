...
import { getAssociatedTokenAddress, createAssociatedTokenAccountInstruction } from "@/lib/constants"
...
export const POST = async (req: NextRequest, { params }: { params: { uniqueid: string } }) => {
  try {
    ...
    if (amountParam) {
      try {
        amount = Number.parseFloat(amountParam) || amount
      } catch (err) {
    ...
    const bondingCurveATA = await getAssociatedTokenAddress(PUMP_MINT, bondingCurve)

    const userATA = await getAssociatedTokenAddress(PUMP_MINT, account)

    const transaction = new Transaction()

    // Add instruction to create ATA if it doesn't exist
    transaction.add(createAssociatedTokenAccountInstruction(account, userATA, account, PUMP_MINT))
    ...
    if (sol_spent === "Error") {
      ...
      if (
        blinkData?.commission &&
        blinkData?.commission === "yes" &&
        blinkData?.percentage &&
        blinkData.percentage > 0
      ) {
...
async function estimateTotalTransactionCost(connection: Connection, transaction: Transaction, wallet: PublicKey) {
  ...
  const simulationResult = await connection.simulateTransaction(transaction, undefined, [wallet])
  ...
  const quoteResponse = await (
    await fetch(
      `https://quote-api.jup.ag/v6/quote?inputMint=So11111111111111111111111111111111111111112&outputMint=${
        blinkData.mint
      }&amount=${amount * 10 ** decimals}&slippageBps=300&swapMode=ExactOut`,
    )
  ).json()
